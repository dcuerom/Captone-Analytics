...
import { getBackendMapUrlForRun, getRoutesGeometry, type RoutesGeometryPayload } from "../data/api";
...
function clusterIndex(value: unknown): number {
...
export function FleetMap() {
  ...
  const centerLng = geometry?.depot?.coordinates?.[1] ?? -70.6693;
  ...
  useEffect(() => {
    const loadGeometry = async () => {
      try {
        const payload = await getRoutesGeometry(effectiveRunId || undefined);
        setGeometry(payload);
        setGeometryError(null);
      } catch (err) {
        const message = err instanceof Error ? err.message : "No se pudo cargar geometria de rutas";
        setGeometry(null);
  ...
  const geometryByVehicle = useMemo(() => {
    const out = new Map<string, RoutesGeometryPayload["routes"][number]>();
    if (!geometry) {
      return out;
    }
    for (const route of geometry.routes) {
      out.set(route.vehicleId, route);
    }
    return out;
  }, [geometry]);

  const filteredRoutes = selectedRoute === "all" ? run.routes : run.routes.filter((r) => r.vehicleId === selectedRoute);
    ...
    <div className="h-screen flex flex-col">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-slate-600">
              {run.name} - {run.totalVehiclesUsed} vehiculos activos
            </p>
          ...
          <div className="flex items-center gap-3">
            <Button variant="outline" onClick={() => setUseBackendMap((v) => !v)}>
              {useBackendMap ? "Mapa UI (fallback)" : "Mapa Backend (HTML)"}
            </Button>
            <div className="w-64">
      ...
      <div className="flex-1 flex">
        <div className="flex-1 relative">
          {useBackendMap ? (
            <iframe
              src={getBackendMapUrlForRun(effectiveRunId)}
              title="Mapa Backend Calles Reales"
              className="w-full h-full border-0"
              onError={() => {
                setBackendMapError("No se pudo renderizar el HTML del mapa.");
                setUseBackendMap(false);
              }}
            />
          ) : (
            <MapContainer center={[centerLat, centerLng]} zoom={11} style={{ height: "100%", width: "100%" }}>
              ...
              {geometry?.depot?.coordinates && (
                <CircleMarker
                  center={[geometry.depot.coordinates[0], geometry.depot.coordinates[1]]}
                  radius={10}
                  pathOptions={{ color: "#1e293b", fillColor: "#3b82f6", fillOpacity: 0.9, weight: 2 }}
                >
                  <Popup>
                    <div className="min-w-48">
                      <p className="font-semibold text-slate-900">Deposito</p>
                      <p className="text-xs text-slate-600">{geometry.depot.address ?? "Sin direccion"}</p>
                    </div>
                  </Popup>
                </CircleMarker>
              )}
              ...
              {filteredRoutes.map((route) => {
                const routeGeom = geometryByVehicle.get(route.vehicleId);
                const routeOrders = route.stops.map((stop) => orders.find((o) => o.id === stop.orderId)).filter((o) => o !== undefined);
                const cIdx = clusterIndex(route.cluster);
                const routeColor = CLUSTER_COLORS[cIdx % CLUSTER_COLORS.length];

                const fallbackCoords = routeOrders.map((order) => [order!.lat, order!.lng] as [number, number]);
                const geometryCoords = routeGeom?.coordinates?.map((coord) => [coord[0], coord[1]] as [number, number]) ?? [];
                const coordinates = geometryCoords.length > 1 ? geometryCoords : fallbackCoords;

                return (
                  <div key={route.vehicleId}>
                    {coordinates.length > 1 && (
                      <Polyline positions={coordinates} pathOptions={{ color: routeColor, weight: 3, opacity: 0.7 }} />
                    )}
                              ...
                              <div className="space-y-1 text-sm">
                                ...
                                <div className="flex items-start gap-2 pt-2 border-t">
                                  ...
                                  <div>
                                    ...
                                    {stop.waitTimeMinutes > 0 && <p className="text-xs text-amber-600 mt-1">Espera: {stop.waitTimeMinutes} min</p>}
                                  </div>
                                </div>

                                <div className="pt-2 border-t">
                                  <p className="text-xs text-slate-600">
                                    Carga: {order.volumeM3}m3 / {order.weightKg}kg
                                  </p>
                                  <p className="text-xs text-slate-600">
          ...
          <div className="p-4 space-y-4">
            <div>
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-center gap-2 text-sm">
                  ...
                  <span className="text-slate-700">Violacion de ventana</span>
                </div>
                {Array.from(new Set(run.routes.map((r) => clusterIndex(r.cluster)))).map((cluster) => (
                  <div key={cluster} className="flex items-center gap-2 text-sm">
                    <div className="w-4 h-1 rounded" style={{ backgroundColor: CLUSTER_COLORS[cluster % CLUSTER_COLORS.length] }} />
                    <span className="text-slate-700">Ruta Cluster {cluster}</span>
                  </div>
                ))}
              </div>
            </div>
                      ...
                      <div className="grid grid-cols-2 gap-2 text-xs">
                        ...
                        <div>
                          ...
                          <p className="font-medium text-slate-900">
                            {route.onTimeDeliveries}/{route.ordersDelivered}
                          </p>
                        </div>
                        <div>
                          <p className="text-slate-500">Utilizacion</p>
            ...
            <Card className="bg-blue-50 border-blue-200">
              <CardContent className="p-4">
                <h4 className="font-medium text-blue-900 text-sm mb-2">Integracion de Mapa</h4>
                <p className="text-xs text-blue-800">
                  Prioridad de visualizacion: 1) HTML backend por runId, 2) geometria JSON del backend, 3) lineas rectas con coordenadas de pedidos.
                </p>
              </CardContent>
            </Card>
          </div>
