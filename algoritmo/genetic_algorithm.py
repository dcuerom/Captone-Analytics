...
OrquestaciÃ³n hÃ­brida Savings + GA (PyMoo) para TDVRPTW.
"""
...
def _load_input_dataframe(input_csv_path: Optional[str]) -> tuple[pd.DataFrame, str]:
    ...
    raise FileNotFoundError(f"No se encontrÃ³ una fuente de datos vÃ¡lida: {candidate_eda} ni {candidate_sim}")
...
def _notify_progress(
    ...
    try:
        ...
        # El progreso no debe detener el pipeline de optimizaciÃ³n.
        pass
...
def optimizar_pymoo_ga(
    ...
    """Ejecuta Savings + GA de PyMoo sobre un clÃºster."""
    ...
    if res.X is None:
        ...
        raise ValueError(f"El GA no generÃ³ poblaciÃ³n vÃ¡lida para clÃºster {cluster_idx}.")
...
def disparar_rutina_ga(
    ...
    if df.empty:
        raise ValueError(f"La fuente de datos estÃ¡ vacÃ­a: {data_path}. Carga un CSV con pedidos antes de optimizar.")
    ...
    if 'fecha_entrega' in df.columns:
        ...
        if df_filtro.empty:
            ...
            available_text = ", ".join(available_dates[:12]) if available_dates else "sin fechas vÃ¡lidas"
            raise ValueError(
    ...
    print(f"Pedidos capturados para {fecha_target} (DÃ­a {dia_semana_target}): {len(df_filtro)}")
    ...
    df_filtro.rename(columns={'Latitud': 'latitud', 'Longitud': 'longitud', 'DirecciÃ³n': 'direccion_ruteo'}, inplace=True)
    ...
    resolved_depot_address = str(depot_address).strip() if depot_address else str(cfg.get("depot_address", "")).strip()
    if not resolved_depot_address:
        resolved_depot_address = str(DEFAULT_OPTIMIZATION_CONFIG.get("depot_address", "")).strip()
    if not resolved_depot_address:
        raise ValueError("No se definio una direccion de deposito valida (depot_address).")
    ...
    matrices_km_o_m, rutas_dict, G, depot_coords = execute_vrp_pipeline(
        ...
        depot_address=resolved_depot_address,
    ...
    graph_path = os.path.join(base_dir, "grafo", "santiago_routing_graph.graphml")
    ...
    _notify_progress(progress_callback, "completed", "OptimizaciÃ³n completada.", 100.0)

    print("[Ã‰xito] OptimizaciÃ³n y AsignaciÃ³n de Flota finalizada.")
    return {
        "run_id": run_id,
        "fecha_target": fecha_target,
