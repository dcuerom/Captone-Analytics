...
1. **Clústering Operacional Basado en Densidad (DBSCAN Clásico)**
   - El sistema opera bajo un enfoque *Cluster-First*, partiendo el plano entero en archipiélagos aislados donde se rutea internamente. Esto se hace midiendo la densidad Euclidiana relativa de la cartografía y separando la malla allí donde la continuidad de vecinos se interrumpe, omitiendo métodos forzados de K centros (`K-Means`). 
